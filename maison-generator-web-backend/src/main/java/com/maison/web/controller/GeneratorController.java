...
import com.maison.maker.meta.MetaValidator;
...
import com.maison.web.service.GeneratorService;
...
public class GeneratorController {
    ...
    public void useGenerator(@RequestBody GeneratorUseRequest generatorUseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // 执行脚本
        // 找到脚本文件所在路径
        // 注意: mac 找generator; win 找 generator.bat
        ...
        // 注意: 兼容 mac/linux 与win
        String scriptCommand;
        if (System.getProperty("os.name").toLowerCase().contains("win")) {
            // Windows 使用绝对路径
            scriptCommand = scriptFile.getAbsolutePath().replace("\\", "/");
        } else {
            // Mac/Linux 使用 "./脚本名"
            scriptCommand = "./" + scriptFile.getName();
        }
        String[] commands = new String[] {scriptCommand, "json-generate", "--file=" + dataModelFilePath};
