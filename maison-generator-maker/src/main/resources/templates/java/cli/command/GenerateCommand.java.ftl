...
<#macro generateOption indent modelInfo>
    ...
    ${indent}private ${modelInfo.type} ${modelInfo.fieldName} <#if modelInfo.defaultValue??> = <#if modelInfo.defaultValue?is_boolean>${modelInfo.defaultValue?c}<#else>"${modelInfo.defaultValue}"</#if></#if>;
</#macro>
