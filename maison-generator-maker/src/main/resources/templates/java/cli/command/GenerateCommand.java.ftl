...
public class GenerateCommand implements Callable<Integer> {
    <#list modelConfig.models as modelInfo>
        ...
        private ${modelInfo.type} ${modelInfo.fieldName} <#if modelInfo.defaultValue??> = <#if modelInfo.defaultValue?is_boolean>${modelInfo.defaultValue?c}<#else>"${modelInfo.defaultValue}"</#if></#if>;
    </#list>
